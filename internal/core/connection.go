...
func (h *ConnectionHandler) processASRResultQueueCoroutine() {
	...
	for {
		...
		case asrText := <-h.asrResultQueue:
			h.LogInfo(fmt.Sprintf("[协程] [ASR队列] 处理ASR结果: %s", internalutils.SanitizeForLog(asrText)))

			// 检查是否是重复的唤醒词处理
			if internalutils.IsWakeUpWord(asrText) {
				now := time.Now()
			...
			if err := h.handleChatMessage(context.Background(), asrText); err != nil {
				...
				h.LogDebug(fmt.Sprintf("[协程] [ASR队列] ASR结果处理完成: %s", internalutils.SanitizeForLog(asrText)))
			}
		}
	}
}
...
func (h *ConnectionHandler) OnAsrResult(result string, isFinalResult bool) bool {
	...
	if h.clientListenMode == "auto" {
		...
		h.LogInfo(fmt.Sprintf("[ASR] [识别结果 %s/%s]", h.clientListenMode, internalutils.SanitizeForLog(result)))
		// 将ASR结果放入队列，避免并发处理
		select {
		case h.asrResultQueue <- result:
			h.LogDebug(fmt.Sprintf("[ASR] [队列] 已将结果放入队列: %s", internalutils.SanitizeForLog(result)))
		default:
			h.LogWarn(fmt.Sprintf("[ASR] [队列] 队列已满，丢弃结果: %s", internalutils.SanitizeForLog(result)))
		}
		return true
		...
		if isFinalResult {
			...
			case h.asrResultQueue <- h.client_asr_text:
				h.LogDebug(fmt.Sprintf("[ASR] [队列] 已将手动结果放入队列: %s", internalutils.SanitizeForLog(h.client_asr_text)))
			default:
				h.LogWarn(fmt.Sprintf("[ASR] [队列] 队列已满，丢弃手动结果: %s", internalutils.SanitizeForLog(h.client_asr_text)))
			}
			return true
		}
		return false
		...
		select {
		case h.asrResultQueue <- result:
			h.LogDebug(fmt.Sprintf("[ASR] [队列] 已将实时结果放入队列: %s", internalutils.SanitizeForLog(result)))
		default:
			h.LogWarn(fmt.Sprintf("[ASR] [队列] 队列已满，丢弃实时结果: %s", internalutils.SanitizeForLog(result)))
		}
		return true
	}
	return false
...
func (h *ConnectionHandler) QuitIntent(text string) bool {
	...
	cleand_text := internalutils.RemoveAllPunctuation(text) // 移除标点符号，确保匹配准确
	// 检查是否包含退出命令
...
func (h *ConnectionHandler) handleChatMessage(ctx context.Context, text string) error {
	...
	// 记录正在处理对话的状态
	h.LogInfo(fmt.Sprintf("[对话] [开始处理] 文本: %s", internalutils.SanitizeForLog(text)))
...
clearedAudioQueue:
	...
	h.LogInfo(fmt.Sprintf("[聊天] [消息 %s]", internalutils.SanitizeForLog(text)))
...
func (h *ConnectionHandler) genResponseByLLM(ctx context.Context, messages []providers.Message, round int) error {
	...
	for response := range responses {
		...
		if content != "" {
			...
			// 处理分段
			fullText := internalutils.JoinStrings(responseMessage)
			...
			// 按标点符号分割
			if segment, charsCnt := internalutils.SplitAtLastPunctuation(currentText); charsCnt > 0 {
				textIndex++
	...
	if toolCallFlag {
		bHasError := false
		if functionID == "" {
			a := internalutils.Extract_json_from_string(contentArguments)
			if a != nil {
	...
	// 处理剩余文本
	fullResponse := internalutils.JoinStrings(responseMessage)
	...
	// 分析回复并发送相应的情绪
	content := internalutils.JoinStrings(responseMessage)
	// 为对话历史和事件发布移除表情符号，保持对话历史的清洁
	cleanContent := internalutils.RemoveAllEmoji(content)
...
func (h *ConnectionHandler) handleWakeUpMessage(ctx context.Context, text string) error {
	h.LogInfo(fmt.Sprintf("[唤醒] [快速响应] 检测到唤醒词: %s", internalutils.SanitizeForLog(text)))
	...
	if h.userID == "" {
		...
		if enabled && len(words) > 0 {
			responseText = internalutils.RandomSelectFromArray(words)
		} else {
			// 如果关闭快速回复，使用LLM生成回复
	...
	h.dialogueManager.Put(chat.Message{
		Role:    "assistant",
		Content: internalutils.RemoveAllEmoji(responseText), // 移除表情符号
	})
...
func (h *ConnectionHandler) SystemSpeak(text string) error {
	...
	texts := internalutils.SplitByPunctuation(text)
	index := 0
...
func (h *ConnectionHandler) SpeakAndPlay(text string, textIndex int, round int) error {
	...
	text = internalutils.RemoveMarkdownSyntax(text) // 移除Markdown语法
	if text == "" {
		// 如果清理后的文本为空，可能是纯表情符号或纯Markdown，跳过语音合成
		h.logger.Debug("SpeakAndPlay 跳过空文本分段，原始文本: %s, 索引: %d", internalutils.SanitizeForLog(originText), textIndex)
		return nil // 不报错，直接跳过
	}

	if atomic.LoadInt32(&h.serverVoiceStop) == 1 { // 服务端语音停止
		h.LogInfo(fmt.Sprintf("speakAndPlay 服务端语音停止, 不再发送音频数据：%s", internalutils.SanitizeForLog(text)))
		text = ""
		return errors.New("服务端语音已停止，无法合成语音")
	}

	if len(text) > 255 {
		h.logger.Warn("文本过长，超过255字符限制，截断合成语音: %s", internalutils.SanitizeForLog(text))
		text = text[:255] // 截断文本
	}
...
func (h *ConnectionHandler) cleanTTSAndAudioQueue(bClose bool) error {
	...
	for {
		select {
		case task := <-h.ttsQueue:
			h.LogInfo(fmt.Sprintf("[%sTTS队列] 丢弃待处理任务: %s", msgPrefix, internalutils.SanitizeForLog(task.text)))
		default:
			// 队列已清空，退出循环
...
clearAudioQueue:
	...
	for {
		select {
		case task := <-h.audioMessagesQueue:
			h.LogInfo(fmt.Sprintf("[%s音频队列] 丢弃待播放任务: %s", msgPrefix, internalutils.SanitizeForLog(task.text)))
			// 根据配置删除被丢弃的音频文件
...
func (h *ConnectionHandler) genResponseByVLLM(ctx context.Context, messages []providers.Message, imageData domainimage.ImageData, text string, round int) error {
	...
	for response := range responses {
		...
		// 按标点符号分割
		if segment, chars := internalutils.SplitAtLastPunctuation(currentText); chars > 0 {
			textIndex++
			h.tts_last_text_index = textIndex
	...
	// 处理剩余文本
	remainingText := internalutils.JoinStrings(responseMessage)[processedChars:]
	if remainingText != "" {
		textIndex++
		h.tts_last_text_index = textIndex
		h.SpeakAndPlay(remainingText, textIndex, round)
