...
func TestModelSelectionManager(t *testing.T) {
	...
	// 测试初始化默认选择
	err = manager.InitDefaultSelection("admin")
	if err != nil {
		t.Fatalf("Failed to init default selection: %v", err)
	}

	// 测试获取选择
	selection, err := manager.GetModelSelection("admin")
	if err != nil {
		t.Fatalf("Failed to get model selection: %v", err)
	}

	if selection.UserID != "admin" {
		t.Errorf("Expected user ID 'admin', got '%s'", selection.UserID)
	}
	...
	// 测试更新选择
	err = manager.UpdateModelSelection("admin", "GoSherpaASR", "DoubaoTTS", "OllamaLLM", "ChatGLMVLLM")
	if err != nil {
