...
func TestNewEncoder(t *testing.T) {
	...
	_, err = NewEncoder(WithSampleRate(44100))
...
func testEncoderInterleavedSineS16LEAtRate(sampleRate, sampleCount, channels int) []byte {
	pcm := make([]byte, sampleCount*channels*2)
	for i := range sampleCount {
		for ch := 0; ch < channels; ch++ {
			freq := 440.0 + float64(ch)*220
			sample := int16(math.Round(math.Sin(2*math.Pi*freq*float64(i)/float64(sampleRate)) * 16000))
			binary.LittleEndian.PutUint16(pcm[(i*channels+ch)*2:], uint16(sample)) //nolint:gosec // G115: little-endian s16 round-trip.
		}
	}
